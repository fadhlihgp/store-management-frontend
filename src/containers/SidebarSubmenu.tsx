...
function SidebarSubmenu(route: Route){
    ...
    useEffect(() => {
        if(route.submenu?.filter(m => {return m.path === location.pathname})[0])setIsExpanded(true)
    }, [])
    ...
    return (
        <div className='flex flex-col'>
            ...
            {/** Route header */}
            <div className='w-full block flex justify-between' onClick={() => setIsExpanded(!isExpanded)}>
            ...
            <div className={` w-full `+ (isExpanded ? "" : "hidden")}>
                ...
                {
                    route.submenu?.map((m, k) => {
                        return(
                            <li key={k}>

                                <Link to={m.path}>
                                    <div dangerouslySetInnerHTML={{ __html: m.icon }} />
                                     {m.name}
                                    {
